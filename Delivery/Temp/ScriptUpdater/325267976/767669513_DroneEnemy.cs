            ...
            if (Time.time - startTime > startStallTime)
            {
                ...
                if (distanceToPlayer > damageDistance)
                {
                    ...
                    if (distanceToPlayer <= maxFollowDistance && inSameFloor)
                    {
                        ...
                        RaycastHit2D hit = Physics2D.Raycast(transform.position, GetComponent<Rigidbody>().velocity.normalized, bounceOnBoundsDistance, Physics2D.DefaultRaycastLayers, 0, (int)QueryTriggerInteraction.Collide);


                        if ((hit.collider != null && hit.collider.GetComponent<Floor>()) || (hit.collider != null && hit.collider.GetComponent<Ladder>()))
                        {
                            Vector2 newDirection = Vector2.Reflect(GetComponent<Rigidbody>().velocity.normalized, hit.normal);
                            newDirection.y = 0;
                            GetComponent<Rigidbody>().velocity = newDirection.normalized * enemySpeed;
                        }
                        else
                        {
                            //chase!
                            ...
                            moveDirection.y = 0;
                            GetComponent<Rigidbody>().velocity = moveDirection.normalized * enemySpeed;
                        }
                    }
                    else
                    {
